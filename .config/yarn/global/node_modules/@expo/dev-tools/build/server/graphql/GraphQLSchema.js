...
const resolvers = {
    ...
    Project: {
        ...
        async config(project) {
            try {
                const { exp } = (0, config_1.getConfig)(project.projectDir);
                return exp;
            }
            catch (error) {
        ...
        async interstitialPageUrl(project) {
            const { devClient } = await xdl_1.ProjectSettings.readAsync(project.projectDir);
            if (xdl_1.Env.isInterstitiaLPageEnabled() &&
                !devClient &&
                (0, xdl_1.isDevClientPackageInstalled)(project.projectDir)) {
                return xdl_1.UrlUtils.constructLoadingUrlAsync(project.projectDir, null);
            }
            return null;
        },
    },
    ...
    Mutation: {
        ...
        async optimizeAssets(parent, { settings }, context) {
            const currentProject = context.getCurrentProject();
            await (0, spawn_async_1.default)('npx', ['expo-optimize'], {
        ...
        async setProjectConfig(parent, { input }, context) {
            ...
            const { exp } = await (0, config_1.writeConfigJsonAsync)(currentProject.projectDir, filteredInput);
            return {
                ...currentProject,
                config: exp,
            };
    ...
    Subscription: {
        messages: {
            subscribe(parent, { after }, context) {
                ...
                const iterator = (0, mergeAsyncIterators_1.default)(issueIterator, messageIterator);
                return {
                    async next() {
